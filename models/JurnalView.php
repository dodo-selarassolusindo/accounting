...
class JurnalView extends Jurnal
{
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (jurnald)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("jurnald", explode(",", $this->getCurrentDetailTable()))) {
            $jurnald = new JurnaldList();
            $rsdetail = $jurnald->loadRs($jurnald->getDetailFilterFromSession(), $jurnald->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($jurnald);
                    $jurnald->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
