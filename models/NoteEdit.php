...
class NoteEdit extends Note
{
    ...
    // CSS class/style
    public $CurrentPageName = "noteedit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("NoteID") ?? Key(0) ?? Route(2)) !== null) {
                $this->NoteID->setQueryStringValue($keyValue);
                $this->NoteID->setOldValue($this->NoteID->QueryStringValue);
            } elseif (Post("NoteID") !== null) {
                $this->NoteID->setFormValue(Post("NoteID"));
                $this->NoteID->setOldValue($this->NoteID->FormValue);
            } else {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("notelist"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "notelist") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "notelist") {
                            ...
                            $returnUrl = "notelist"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    {
        global $CurrentForm;
        $this->NoteID->CurrentValue = $this->NoteID->FormValue;
