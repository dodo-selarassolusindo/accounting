...
class JurnalkasView extends Jurnalkas
{
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (jurnalkasd)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("jurnalkasd", explode(",", $this->getCurrentDetailTable()))) {
            $jurnalkasd = new JurnalkasdList();
            $rsdetail = $jurnalkasd->loadRs($jurnalkasd->getDetailFilterFromSession(), $jurnalkasd->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($jurnalkasd);
                    $jurnalkasd->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
